<!-- CHART.JS -->
<script src="<?=BASEASSETS;?>vendor/chart.js/Chart.min.js"></script>

<script src="<?=BASEASSETS;?>vendor/DataTables1/Buttons-1.6.1/js/buttons.bootstrap4.min.js"></script>
<script src="<?=BASEASSETS;?>vendor/DataTables/datatables.min.js"></script>
<script src="<?=BASEASSETS;?>vendor/datatables/dataTables.bootstrap4.min.js"></script>

<script src="<?=BASEASSETS;?>js/pages/product/check-current-inventory.js"></script>